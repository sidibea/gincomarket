{if count($products) > 0 && $products != null}
	...
	<div class="pos_featured_product">
		<div class="title_block">
			<h3>{l s='Featured Products' mod='posfeatureproduct'}</h3>
			<div class="navi">
							...
							<div class="home_tab_img">
								...
								{if isset($quick_view) && $quick_view}
									<a 	title="{l s='Quick view' mod='posfeatureproduct'}"
										...
										<i class="icon-external-link"></i>{l s='Quick view' mod='posfeatureproduct'}
									</a>
								{/if}
							</div>
							<div class="home_tab_info">
								...
								<div class="btn_content">
										<a class="add_to_compare" 
											href="{$product.link|escape:'html':'UTF-8'}" 
											title="{l s='Add to compare' mod='posfeatureproduct'}"
											data-id-product="{$product.id_product}">
											<i class="icon-signal"></i>
										</a>
										<a 	title="{l s='Add to wishlist' mod='posfeatureproduct'}"
											...
											{if ($product.allow_oosp || $product.quantity > 0)}
												{if isset($static_token)}
													<a class="exclusive ajax_add_to_cart_button btn btn-default" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$product.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart' mod='posfeatureproduct'}" data-id-product="{$product.id_product|intval}">
														<i class="icon-shopping-cart"></i>{l s='Add to cart' mod='posfeatureproduct'}
													</a>
												{else}

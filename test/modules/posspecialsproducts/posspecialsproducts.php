...
class posspecialsproducts extends Module
{
	...
	function __construct()
	{
		...
		$this->displayName = ('Pos Specials products');
		$this->description = $this->l('Adds a block displaying your current discounted products');
	}

	function install()
	{
		if (!Configuration::updateValue('SPECIAL_PRODUCTS_NBR', 20) OR !parent::install() OR !$this->registerHook('blockPosition1') OR !$this->registerHook('leftcolumn') OR !$this->registerHook('header'))
			return false;
		return true;
	}

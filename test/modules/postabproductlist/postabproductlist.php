...
class postabproductlist extends Module {
	var $_postErrors  = array();
	public function __construct() {
		$this->name 		= 'postabproductlist';
		$this->tab 			= 'front_office_features';
		$this->version 		= '1.5';
		$this->author 		= 'posthemes';
		$this->displayName 	= $this->l('Product List Tabs Slider');
		$this->description 	= $this->l('Product List Tabs Slider');
	...
	public function install() {
      ...
      public function uninstall() {
        $this->_clearCache('postabproductlist.tpl');
    ...
    public function hookHeader($params){
        if ($this->psversion()==5){
            $this->context->controller->addCSS(($this->_path).'postabproductlist.css', 'all');

        } else {
			Tools::addCSS(($this->_path).'postabproductlist.css');
	...
	public function hookBlockPosition2($params) {
			...
			$newProducts = Product::getNewProducts((int) Context::getContext()->language->id, 0, ($nb ? $nb : 20));
			$specialProducts = Product::getPricesDrop((int) Context::getContext()->language->id, 0, ($nb ? $nb : 20));
			ProductSale::fillProductSales();
			$bestseller =  $this->getBestSales ((int) Context::getContext()->language->id, 0, ($nb ? $nb : 20), null,  null);
			$category = new Category(Context::getContext()->shop->getCategory(), (int) Context::getContext()->language->id);
         	$featureProduct = $category->getProducts((int) Context::getContext()->language->id, 0, ($nb ? $nb : 20));
			...
			if(Configuration::get($this->name . '_show_sale')) {
				$productTabslider[] = array('id'=> 'special_product','name' => $this->l('Sale Products'), 'productInfo' =>  $specialProducts);
			}
				$options = array(
		...
		return $this->display(__FILE__, 'postabproductlist.tpl');
	}
	
	  public function getContent() {
        $output = '<h2>' . $this->displayName . '</h2>';
        if (Tools::isSubmit('submitpostabproductlist')) {
	 ...
	 $tabEffect = array();
		...
		<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
                  <fieldset>
                    ...
                    <input type="submit" name="submitpostabproductlist" value="'.$this->l('Update').'" class="button" />
                     </fieldset>
		</form>';
		return $this->_html;
	}

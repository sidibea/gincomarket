...
class Posmegamenu extends Module {
    ...
    public function __construct() {
        ...
        $this->displayName = $this->l('Pos Megamenu Customer');
    ...
    public function install() {
        ...
        Configuration::updateValue($this->name . '_top_offset', 65);
    ...
    public function getStaticBlockContent($blockId = NULL, $task = NULL) {
        ...
        if (count($staticBlock) > 0) {
            ...
            $description = str_replace('/pos_dilima/',__PS_BASE_URI__,$description);
            $html .= $description;
        }
        if ($task == 'item') {
    ...
    public function drawCustomMenuBlock($blockId, $bc) {
        ...
        $blockHtml = str_replace('/pos_dilima/',__PS_BASE_URI__,$bc['description']);
        $drawPopup = $blockHtml;
        if ($drawPopup) {
		...
		foreach ($menu_items as $item)
		{
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'CMS':
					...
					if (count($cms))
					$this->_menuLink .= '<div class ="pt_menu_cms pt_menu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($cms[0]['link']).'"><span>'.$cms[0]['meta_title'].'</span></a></div></div>'.PHP_EOL;
				break;
			    
				case 'CMS_CAT':
					$category = new CMSCategory((int)$id, (int)$id_lang);
					if (Validate::isLoadedObject($category))
					$this->_menuLink .= '<div class ="pt_menu_cms pt_menu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($category->getLink()).'"><span>'.$category->name.'</span></a>';
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						$this->_menuLink .= '<div id ="pt_menu_link" class ="pt_menu"><div class="parentMenu" ><a href="'.Tools::HtmlEntitiesUTF8($link[0]['link']).'"'.(($link[0]['new_window']) ? ' target="_blank"': '').'><span>'.$link[0]['label'].'</span></a></div></div>'.PHP_EOL;
					}
					break;

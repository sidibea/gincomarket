...
FROM `'._DB_PREFIX_.'product` p ' . ((intval($cookie->profile) == intval(Configuration::get('AGILE_MS_PROFILE_ID')))? 'INNER JOIN `'._DB_PREFIX_.'product_owner` po ON (p.id_product=po.id_product AND po.id_owner=' . $cookie->id_employee . ')' :'')  . '

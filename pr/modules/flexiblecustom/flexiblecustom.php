...
class FlexibleCustom extends Module
{
    ...
    public function getGroupProducts($group, $module){
        ...
        if($group['type'] == 'auto'){
            if($params->orderValue == 'sales'){
                $products =  $this->getProductsOrderSales($langId, $group['categoryId'], $params, $group['maxItem'], false, true, false, 0);
            }elseif($params->orderValue == 'price'){
                $products = $this->getProductsOrderPrice($langId, $group['categoryId'], $params, $group['maxItem'], false, true, false, 0);
            }elseif($params->orderValue == 'discount'){
                $products = $this->getProductsOrderSpecial($langId, $group['categoryId'], $params, $group['maxItem'], false, true, false, 0);
            }elseif($params->orderValue == 'add'){
                $products = $this->getProductsOrderAddDate($langId, $group['categoryId'], $params, $group['maxItem'], false, true, false, 0);
            }elseif($params->orderValue == 'rand'){
                $products = $this->getProductsOrderRand($langId, $group['categoryId'], $params, $group['maxItem'], false, true);
            }
        }else{
        	$products = array();
   ...
   public function getProductsOrderSales($id_lang, $categoryId = 0, $params = null, $limit, $short = true, $getProperties = true, $total=false, $offset = 0){
   		if(!$categoryId) $categoryId = Configuration::get('PS_HOME_CATEGORY');
        ...
        if (Group::isFeatureActive())
		{
			...
			$where .= 'AND p.`id_product` IN (
				...
				WHERE cp.id_category = '.$categoryId.' AND cg.`id_group` '.(count($groups) ? 'IN ('.implode(',', $groups).')' : '= 1').'
			)';
		}else{
            $where .= 'AND p.`id_product` IN (
				SELECT cp.`id_product`
    ...
    function getProductsOrderPrice($id_lang, $categoryId=0, $params = null, $limit, $short = true, $getProperties = true, $total = false, $offset = 0){
    	if(!$categoryId) $categoryId = Configuration::get('PS_HOME_CATEGORY');
        $context = Context::getContext();
        $order_by = 'price';
        $order_way = 'DESC';
    ...
    function getProductsOrderRand($id_lang, $categoryId=0, $params = null, $limit, $short = true, $getProperties = true){
    	if(!$categoryId) $categoryId = Configuration::get('PS_HOME_CATEGORY');
        $context = Context::getContext();
        $order_by = 'RAND()';
        //$order_way = $params->orderType;
    ...
    function getProductsOrderAddDate($id_lang, $categoryId=0, $params = null, $limit, $short = true, $getProperties = true, $total=false, $offset = 0){
    	if(!$categoryId) $categoryId = Configuration::get('PS_HOME_CATEGORY');
        $context = Context::getContext();
        $order_by = 'date_add';
        $order_way = 'DESC';
    ...
    public function getProductsOrderSpecial($id_lang, $categoryId=0, $params = null, $limit, $short = true, $getProperties = true, $total = false, $offset = 0)
	{
		if(!$categoryId) $categoryId = Configuration::get('PS_HOME_CATEGORY');
    ...
    function getProductList($id_lang, $categoryId=0, $notIn = '', $keyword = '', $getTotal = false, $offset=0, $limit=10){
		if(!$categoryId) $categoryId = Configuration::get('PS_HOME_CATEGORY');
        $where = "";
        //if($arrCategory){ $catIds = implode(', ', $arrCategory)}
        if (Group::isFeatureActive())
		{
			...
			$where .= ' AND p.`id_product` IN (
				...
				FROM `'._DB_PREFIX_.'category_group` cg
				LEFT JOIN `'._DB_PREFIX_.'category_product` cp ON (cp.`id_category` = cg.`id_category`)
				WHERE cp.`id_product` Not In ('.$notIn.') AND cp.id_category ='.$categoryId.' AND cg.`id_group` '.(count($groups) ? 'IN ('.implode(',', $groups).')' : '= 1').'
			)';
		}else{
            $where .= ' AND p.`id_product` IN (
				SELECT cp.`id_product`
				FROM `'._DB_PREFIX_.'category_product` cp
				WHERE cp.`id_product` Not In ('.$notIn.') AND cp.id_category ='.$categoryId.')';
		}

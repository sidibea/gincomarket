var RevSliderAdmin = new function(){
		...
		this.initEditSliderView = function(){
			...
			jQuery("#button_export_slider").click(function(){
				...
				var urlAjaxExport = ajaxurl+"?action="+g_uniteDirPlagin+"_ajax_action&client_action=export_slider&dummy="+useDummy;
		...
		t.initSlidesListView = function(sliderID){
			...
			jQuery("#list_slides").delegate(".icon_slide_lang, .icon_slide_lang_add","click",function(event){
				...
				//var posTop = pos[1] - 60;
                                var parentoffset = $(this).parents('li.ui-state-default').offset();
                                var pos = $(this).offset();
                                
				var posLeft = pos.left-20;
				var posTop = parentoffset.top - 55;
			...
			jQuery("#slides_langs_float li a").click(function(){
				...
				if(operation == "edit"){
					...
					var urlSlide = decodeURIComponent(g_patternViewSlide).replace("[slideid]",slideID);
			...
			jQuery(".button_copy_slide").click(function(){
				...
				buttons[textUpdate] = function(){
					...
					var operation = "copy";
					jQuery('input[name="copy_move_operation"]').each(function(){

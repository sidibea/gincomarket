...
<div class="alert error form-error" style="display: block;">
	...
	<ol>
		...
		{if !empty($aError.context)}<li>{l s='Error context' mod='facebookpsconnect'} : {$aError.context|escape:'htmlall':'UTF-8'}</li>{/if}
	</ol>
	{/if}
	{/foreach}
</div>
...
<div id="socialMessage">
	<button name="{$sModuleName|escape:'htmlall':'UTF-8'}Button" value="{l s='Reload' mod='facebookpsconnect'}"  onclick="document.location.href= '{$sLink|escape:'htmlall':'UTF-8'}';">{l s='Reload' mod='facebookpsconnect'}</button>
</div>
{/if}
<?php
class FlexibleBrands extends Module
{
    const INSTALL_SQL_FILE = 'install.sql';
	public $orderValue = array();
    public $orderType = array();
    public $display = array();
    public $groupType = array();
	...
	protected static $arrPosition = array('displayFlexibleBrand');
	...
	*/
	public function install($keep = true)
	{
	   if ($keep)
        ...
        || !$this->registerHook('displayFlexibleBrand')
	...
	public function uninstall($keep = true)
	{
        ...
        if($keep){
			...
			`'._DB_PREFIX_.'flexiblebrand_modules`,
			`'._DB_PREFIX_.'flexiblebrand_module_lang`,
			`'._DB_PREFIX_.'flexiblebrand_groups`,
            `'._DB_PREFIX_.'flexiblebrand_group_lang`,
			`'._DB_PREFIX_.'flexiblebrand_group_products`')) return false;
        }
	...
	public function moduleUpdatePosition(){
		$items = Db::getInstance()->executeS("Select DISTINCT position_name From "._DB_PREFIX_."flexiblebrand_modules  Where `position_name` <> ''");
		if($items){
			foreach ($items as $key => $item) {
				$position = Hook::getIdByName($item['position_name']);
				Db::getInstance()->execute("Update "._DB_PREFIX_."flexiblebrand_modules Set position = '".$position."' Where `position_name` = '".$item['position_name']."'");
			}
		}
	}
	...
	public function getCategoryIds($parentId = 0, $arr=null){
        if($arr == null) $arr = array();
        $items = Db::getInstance()->executeS("Select Distinct id_category From "._DB_PREFIX_."category Where id_parent = $parentId");
        if($items){
            foreach($items as $item){
	...
	public function getModuleByLang($id, $langId=0, $shopId=0){
		...
		$itemLang = Db::getInstance()->getRow("Select module_title, banners From "._DB_PREFIX_."flexiblebrand_module_lang Where module_id = ".$id." AND `id_lang` = ".$langId." AND `id_shop` = ".$shopId);
	...
	public function getGroupByLang($id, $langId=0, $shopId=0){
		if($langId == 0) $langId = Context::getContext()->language->id;
        if($shopId == 0) $shopId = Context::getContext()->shop->id;
		$itemLang = Db::getInstance()->getRow("Select title From "._DB_PREFIX_."flexiblebrand_group_lang Where group_id = ".$id." AND `id_lang` = ".$langId." AND `id_shop` = ".$shopId);
		if(!$itemLang) $itemLang = array('title'=>'');
		return $itemLang;
	}
	function getManufacturerOptions($selected){
		$items = Db::getInstance()->executeS("Select m.id_manufacturer, m.name From "._DB_PREFIX_."manufacturer as m Where m.active = 1");
		$options = '';
		if($items){
			foreach($items as $item){
				if($item['id_manufacturer'] == $selected){
					$options .= '<option selected="selected" value="'.$item['id_manufacturer'].'">'.$item['name'].'</option>';
				}else{
					$options .= '<option value="'.$item['id_manufacturer'].'">'.$item['name'].'</option>';
				}
			}
		}
		return $options;
	}
	function getMenufacturerNameById($id){
		return Db::getInstance()->getValue("Select name From "._DB_PREFIX_."manufacturer Where id_manufacturer = $id");
	}
	function getCategoryNameById($id, $langId, $shopId){
	...
	public function ovicModuleRenderForm($id = 0){
		$shopId = Context::getContext()->shop->id;
		$item = Db::getInstance()->getRow("Select * From "._DB_PREFIX_."flexiblebrand_modules Where id = ".$id);
		if(!$item){
		...
		//$html['config'] .= '<div class="form-group"><div class="col-lg-12 "><div class="col-lg-12 "><label>'.$this->l('Product order').'</label></div><div class="col-lg-12 "><select class="form-control" name="orderType">'.$this->getOrderTypeOptions($params->orderType).'</select></div></div></div>';
		return $html;
	}

	public function ovicGroupRenderForm($id = 0){
		$shopId = Context::getContext()->shop->id;
		$item = Db::getInstance()->getRow("Select * From "._DB_PREFIX_."flexiblebrand_groups Where id = ".$id);
		if(!$item){
			$item = array('id'=>0, 'module_id'=>0, 'manufacturerId'=>0, 'productCount'=>3, 'maxItem'=>12, 'type'=>'auto', 'ordering'=>1, 'status'=>1, 'icon'=>'', 'iconActive'=>'', 'params'=>'');
			$params = new stdClass();
			$params->displayOnly = 'all';
			$params->orderValue = 'add';
			$params->orderType = 'desc';
		...
		$html .= '<div class="form-group clearfix" style="display:none">
                    <label class="control-label col-lg-3">'.$this->l('Icon').'</label>
                    <div class="col-lg-9 ">
                        <div class="input-group mar-t-10">
                            <input type="text" class="form-control" id="category-icon" name="icon" placeholder="Icon" readonly="readonly" />
		...
		$html .= '<div class="form-group">
                    <label class="control-label col-lg-3">'.$this->l('Manufacturer').'</label>
				    <div class="col-lg-9 ">
                        <select class="form-control" name="manufacturerId">'.$this->getManufacturerOptions($item['manufacturerId']).'</select>
                    </div>
                </div>';
		$html .= '<div class="form-group">
                    <label class="control-label col-lg-3">'.$this->l('Type').'</label>
				    <div class="col-lg-9 "><select name="type" class="form-control" onchange="groupChangeType(this.value)">'.$this->getGroupTypeOptions($item['type']).'</select></div>
                </div>';
	...
	public function getAllGroup($moduleId){
		$langId = Context::getContext()->language->id;
        $shopId = Context::getContext()->shop->id;
		$items = Db::getInstance()->executeS("Select * From "._DB_PREFIX_."flexiblebrand_groups Where module_id = '$moduleId' Order By ordering");
		$html = '';
		if($items){
			foreach($items as $item){
                ...
                $html .= '<tr id="gro_'.$item['id'].'">
                    ...
                    <td>'.$this->getMenufacturerNameById($item['manufacturerId']).'</td>
                    <td class="center">'.$this->groupType[$item['type']].'</td>
	...
	public function getContent()
	{
	    $langId = Context::getContext()->language->id;
        $shopId = Context::getContext()->shop->id;
        $checkUpdate = Db::getInstance()->getRow("Select * From "._DB_PREFIX_."flexiblebrand_modules");
        if($checkUpdate){
            if(!isset($checkUpdate['id_shop'])){
                Db::getInstance()->execute("ALTER TABLE "._DB_PREFIX_."flexiblebrand_modules ADD `id_shop` INT(6) unsigned NOT NULL AFTER `id`");
                Db::getInstance()->execute("Update "._DB_PREFIX_."flexiblebrand_modules Set `id_shop` = ".$shopId);
            }
        }
	   // lang default
		...
		//return $this->display(__FILE__, 'modules.tpl');
	...
	public function hookDisplayFlexibleBrand($params)	{
        return $this->hooks('displayFlexibleBrand', $params);
    }
    ...
    public function hooks($hookName, $param){
        ...
        if (!$this->isCached('flexiblebrand.tpl', $cacheKey)){
			$items = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS("Select DISTINCT m.*, ml.`module_title`, ml.`banners`
				From "._DB_PREFIX_."flexiblebrand_modules as m
				INNER JOIN "._DB_PREFIX_."flexiblebrand_module_lang AS ml On m.id = ml.module_id
				Where m.status = 1 AND m.position = ".$hookId." AND m.id_shop = '".$shopId."' AND ml.id_lang = ".$langId." AND ml.id_shop = ".$shopId."
				Order By m.ordering");
			$results = array();
			if($items){
				foreach($items as $i=>$item){
        ...
        return $this->display(__FILE__, 'flexiblebrand.tpl', $cacheKey);
    }
    public function buildModule($module, $cacheKey = ''){
		...
		if (!$this->isCached($moduleParams->moduleLayout.'.layout.tpl', $cacheKey)){
			$items = Db::getInstance()->executeS("Select g.*, gl.title From "._DB_PREFIX_."flexiblebrand_groups AS g Inner Join "._DB_PREFIX_."flexiblebrand_group_lang AS gl On g.id = gl.group_id Where g.status = 1 AND module_id = ".$module['id']." AND gl.id_lang = ".$langId." AND gl.id_shop = ".$shopId." Order By g.ordering");
			if($items){
				foreach($items as &$item){
					$item['products'] = $this->getGroupProducts($item, $module);
					$item['avatar'] = _THEME_MANU_DIR_.$item['manufacturerId'].'-brand-136x69.jpg';
					$item['manufacturerName'] = $this->getMenufacturerNameById($item['manufacturerId']);
				}
			}else return '';
			$this->context->smarty->assign(array(
				'groups'=>$items,
    ...
    public function getGroupProducts($group, $module){
        ...
        if($group['type'] == 'auto'){
            if($params->orderValue == 'sales'){
                ...
                $products =  $this->getProductsOrderSales($langId, $group['manufacturerId'], $params, $group['maxItem'], false, true, false, 0);
                ...
                $products = $this->getProductsOrderPrice($langId, $group['manufacturerId'], $params, $group['maxItem'], false, true, false, 0);
               ...
               //$total = Functions::getProductsOrderSpecial($langId, $arrSubCategory, $params, $moduleCategoryItem['productCount'], true, false, true);
                $products = $this->getProductsOrderSpecial($langId, $group['manufacturerId'], $params, $group['maxItem'], false, true, false, 0);
               ...
               //$total = Functions::getProductsOrderAddDate($langId, $arrSubCategory, $params, $moduleCategoryItem['productCount'], true, false, true);
                $products = $this->getProductsOrderAddDate($langId, $group['manufacturerId'], $params, $group['maxItem'], false, true, false, 0);
            }elseif($params->orderValue == 'rand'){
                //$total = $moduleCategoryItem['productCount'];
                $products = $this->getProductsOrderRand($langId, $group['manufacturerId'], $params, $group['maxItem'], false, true);
            }
            if(!$products) $products = array();
        }else{
            $items = Db::getInstance()->executeS("Select product_id, ordering From "._DB_PREFIX_."flexiblebrand_group_products Where module_id = ".$module['id']." AND group_id = ".$group['id']." Order By ordering");
            if($items){
                foreach($items as $item){
   ...
   public static function getProductsOrderSales($id_lang, $manufacturerId, $params = null, $limit, $short = true, $getProperties = true, $total=false, $offset = 0){
        $context = Context::getContext();
        $order_by = 'sales';
        $order_way = 'DESC';
        $where = " AND m.id_manufacturer = $manufacturerId ";
        if($params){
    ...
    static function getProductsOrderPrice($id_lang, $manufacturerId, $params = null, $limit, $short = true, $getProperties = true, $total = false, $offset = 0){
        $context = Context::getContext();
        $order_by = 'price';
        $order_way = 'DESC';
        $where = " AND m.id_manufacturer = $manufacturerId ";
    ...
    static function getProductsOrderRand($id_lang, $manufacturerId, $params = null, $limit, $short = true, $getProperties = true){
        $context = Context::getContext();
        $order_by = 'RAND()';
        $order_way = "DESC";
        $where = " AND m.id_manufacturer = $manufacturerId ";
    ...
    static function getProductsOrderAddDate($id_lang, $manufacturerId, $params = null, $limit, $short = true, $getProperties = true, $total=false, $offset = 0){
        $context = Context::getContext();
        $order_by = 'date_add';
        $order_way = 'DESC';
        $where = " AND m.id_manufacturer = $manufacturerId ";
    ...
    public static function getProductsOrderSpecial($id_lang, $manufacturerId, $params = null, $limit, $short = true, $getProperties = true, $total = false, $offset = 0)
	{
    ...
    static function getProductList($id_lang, $manufacturerId, $notIn = '', $keyword = '', $getTotal = false, $offset=0, $limit=10){
        ...
        if (Group::isFeatureActive())
		{
			...
			$where .= ' AND p.`id_product` IN (
				...
				FROM `'._DB_PREFIX_.'category_group` cg
				LEFT JOIN `'._DB_PREFIX_.'category_product` cp ON (cp.`id_category` = cg.`id_category`)
				WHERE cp.`id_product` Not In ('.$notIn.') AND cg.`id_group` '.(count($groups) ? 'IN ('.implode(',', $groups).')' : '= 1').'
			)';
		}else{
            $where .= ' AND p.`id_product` IN (
				SELECT cp.`id_product`
				FROM `'._DB_PREFIX_.'category_product` cp
				WHERE cp.`id_product` Not In ('.$notIn.'))';
		}

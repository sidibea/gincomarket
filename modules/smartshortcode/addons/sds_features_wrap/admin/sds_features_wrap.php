...
<form action="" id="smartshortcode_feature_form" class="smartshortcode_form">
    <div class="form-group"><label>Features Element</label></div>
    <div class="tabfields form-group">
        <label>Column Width:</label>
        <select class="num_col form-control">
            <option value='12'>full</option>
            <option value='6'>one half</option>                
    ...
    <div class="tabfields_wrap">
        ...
        <div class="tabfields form-group">
            ...
            <select class="icon_style form-control">
                <option value="1">Image</option>
                <option value="2">Font Awesome</option>                
            </select>
        </div>
        <div class="tabfields form-group">
            <label>Title:</label><input type="text" placeholder="Title Here" class="feature_title form-control" />
        </div>
        <div class="tabfields form-group">
            <label>Content:</label><textarea placeholder="Content" rows="5" cols="15" class="feature_content form-control"></textarea>
        </div>                     
        <div class="tabfields form-group hidden">
        ...
        <div class="tabfields form-group hidden">
            ...
            <div class="form-group icon-preview">
                <i rel="sds_faw_sample" class="icon-adjust icon-large"></i>                
            </div>
            <div class="form-group">
            <select class="form-control sds_faw_features">            
            ...
            <div class="form-group">
            <select class="form-control sds_faw_icon_size">
    ...
    <div>
        <input type="button" class="btn submit" value="Add New" id="add_new" />
...
$(function(){
    ...
    var styletoggle = function(elem){
        var ssv = elem.find('.icon_style').children('option:selected').val();
        if(ssv == '1'){
            elem.find('.slide_image_src').parent().removeClass('hidden');
            elem.find('.sds_faw_features').parents('.tabfields').addClass('hidden');
        }else{
            elem.find('.slide_image_src').parent().addClass('hidden');
            elem.find('.sds_faw_features').parents('.tabfields').removeClass('hidden');
        }
    }
    styletoggle($('.tabfields_wrap'));
    $(document.body).on('change','.icon_style',function(){
        var parent = $(this).parents('.tabfields_wrap');
        styletoggle(parent);
    });
    $(document.body).on('change','.sds_faw_features',function(){
        var icon = $(this).find('option:selected').val();        
        var parent = $(this).parents('.tabfields_wrap');
        var size = parent.find('.sds_faw_icon_size').children('option:selected').val();
        parent.find('.icon-preview').children('i').attr('class',icon+' '+size);
    });
    $(document.body).on('change','.sds_faw_icon_size',function(){
        var size = $(this).children('option:selected').val();        
        var parent = $(this).parents('.tabfields_wrap');
        var oldclass = parent.find('.icon-preview').children('i').attr('class');
        oldclass = oldclass.substr(0,oldclass.indexOf(' '));        
        parent.find('.icon-preview').children('i').attr('class',oldclass+' '+size);
    });
    ...
    $(document.body).on('click','#add_new',function(){
        ...
        var elemp = parent.prev();
        styletoggle(elemp);
        elemp.find('.slide_image_src').attr('id','slide_image_src_'+c);        
    });
    ...
    var sds_slide_walk = function(elem){
        ...
        elem.find('.tabfields_wrap').each(function(){
            ...
            var content = $(this).find('.feature_content').val().replace(/(\r\n|\n|\r)/gm,"");
            var href = '';
            var col = $('.num_col').children('option:selected').val();
            var icon_style = $(this).find('.icon_style').children('option:selected').val();
            scode += '[sds_feature col="'+col+'" ';
            if(icon_style == '1'){
                href = $(this).find('.slide_image_src').val();
                if(href == ''){
                ...
                var iconclass = $(this).find('.sds_faw_features option:selected').val();
                iconclass += ' '+$(this).find('.sds_faw_icon_size option:selected').val();
                scode += 'iconclass="'+iconclass+'"';
            }
    ...
    $(document.body).on('submit','#smartshortcode_feature_form',function(){
        ...
        scodeb += '[sds_features_wrap]';
        scodea += '[/sds_features_wrap]';

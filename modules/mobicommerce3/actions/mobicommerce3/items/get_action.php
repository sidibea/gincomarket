...
class mobicommerce3_items_get_action extends BaseAction
{
    ...
    {
        if (Tools::getValue($GLOBALS["rIWgbgqIojOWhIMVwwOk"]))
        {
            $sortOptions = explode($GLOBALS["ktwCrReEJlzuBUIjBcCk"], Tools::getValue($GLOBALS["rIWgbgqIojOWhIMVwwOk"]));
        ...
        if (Tools::getValue($GLOBALS["HyoWHiaULyiuurcyZeYC"]))
        {
            switch(Tools::getValue($GLOBALS["HyoWHiaULyiuurcyZeYC"]))
            {
        ...
        $productService = ServiceFactory::factory($GLOBALS["BudoyvNoPXXiWWzhYbHM"]);
        $category_id = Tools::getValue($GLOBALS["PiAcIEATXlQdiYGIyCLU"]);
        $_token = Tools::getValue($GLOBALS["IUyxwiSnFvUOuLSFxbJQ"]);

        $_GET[$GLOBALS["QVWFuLyKVyGmMRnfrqPG"]] = $category_id;

        $info = $productService->getProducts($_REQUEST);
        if($category_id)
        {
            $info[$GLOBALS["PiAcIEATXlQdiYGIyCLU"]] = $category_id;
        }

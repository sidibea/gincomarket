...
// those are the names we'll use.
    ...
    public function encrypt($plaintext) {
        ...
        switch ($this->mode) {
            case CRYPT_RIJNDAEL_MODE_ECB:
                for ($i = 0; $i < Tools::strlen($plaintext); $i+=$block_size) {
            ...
            case CRYPT_RIJNDAEL_MODE_CTR:
                ...
                if ($this->continuousBuffer) {
                    $this->encryptIV = $xor;
                    if ($start = Tools::strlen($plaintext) % $block_size) {
            ...
            case CRYPT_RIJNDAEL_MODE_CFB:
                ...
                for ($i = $start; $i < Tools::strlen($plaintext); $i+=$block_size) {
                    $block = substr($plaintext, $i, $block_size);
                    $xor = $this->_encryptBlock($iv);
                    $iv = $block ^ $xor;
                    if ($continuousBuffer && Tools::strlen($iv) != $block_size) {
                        $buffer = array(
                            'encrypted' => $iv,
    ...
    public function decrypt($ciphertext) {
        ...
        if ($this->paddable) {
            $ciphertext = str_pad($ciphertext, Tools::strlen($ciphertext) + ($this->block_size - Tools::strlen($ciphertext) % $this->block_size) % $this->block_size, chr(0));
        }
        ...
        switch ($this->mode) {
            case CRYPT_RIJNDAEL_MODE_ECB:
                for ($i = 0; $i < Tools::strlen($ciphertext); $i+=$block_size) {
            ...
            case CRYPT_RIJNDAEL_MODE_CTR:
                ...
                if ($this->continuousBuffer) {
                    $this->decryptIV = $xor;
                    if ($start = Tools::strlen($ciphertext) % $block_size) {
                        $buffer = substr($key, $start) . $buffer;
                    }
                }
                break;
            case CRYPT_RIJNDAEL_MODE_CFB:
                if (!empty($buffer['ciphertext'])) {
                    ...
                    if (Tools::strlen($buffer['ciphertext']) == $block_size) {
                ...
                for ($i = $start; $i < Tools::strlen($ciphertext); $i+=$block_size) {
                    $block = substr($ciphertext, $i, $block_size);
                    $plaintext.= $block ^ $xor;
                    if ($continuousBuffer && Tools::strlen($block) != $block_size) {
                        $buffer['ciphertext'].= $block;
                        $block = $xor;
                    } else if (Tools::strlen($block) == $block_size) {
...
// See http://en.wikipedia.org/wiki/Finite_field_arithmetic#Multiplicative_inverse
    ...
    0x7D000000, 0xFA000000, 0xEF000000, 0xC5000000, 0x91000000
        ...
        if (!$this->explicit_key_length) {
            $length = Tools::strlen($this->key) >> 2;
            if ($length > 8) {
                $length = 8;
            } else if ($length < 4) {
                $length = 4;
...
// subWord + invMixColumn + invSubWord = invMixColumn
    ...
    public function _pad($text) {
        ...
        $padLength = $this->block_size - ($length % $this->block_size);
    ...
    public function _unpad($text) {
        ...
        if ($this->paddingMode != CRYPT_PADDING_MODE_ZERO && !$length || $length > $this->block_size) {
            return false;
        }

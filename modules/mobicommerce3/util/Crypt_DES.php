...
class Crypt_DES {
    ...
    public function encrypt($plaintext)
    {
        ...
        if ( CRYPT_DES_MODE == CRYPT_DES_MODE_MCRYPT ) {
            ...
            if ($this->mode != 'ncfb') {
                ...
                if (Tools::strlen($this->enbuffer)) {
                    $ciphertext = $plaintext ^ substr($this->encryptIV, Tools::strlen($this->enbuffer));
                    $this->enbuffer.= $ciphertext;
                    if (Tools::strlen($this->enbuffer) == 8) {
                        $this->encryptIV = $this->enbuffer;
                        $this->enbuffer = '';
                        mcrypt_generic_init($this->enmcrypt, $this->keys, $this->encryptIV);
                    }
                    $plaintext = substr($plaintext, Tools::strlen($ciphertext));
                } else {
                    $ciphertext = '';
                }

                $last_pos = Tools::strlen($plaintext) & 0xFFFFFFF8;
                $ciphertext.= $last_pos ? mcrypt_generic($this->enmcrypt, substr($plaintext, 0, $last_pos)) : '';

                if (Tools::strlen($plaintext) & 0x7) {
                    if (Tools::strlen($ciphertext)) {
        ...
        switch ($this->mode) {
            case CRYPT_DES_MODE_ECB:
                for ($i = 0; $i < Tools::strlen($plaintext); $i+=8) {
            ...
            case CRYPT_DES_MODE_CTR:
                ...
                if ($this->continuousBuffer) {
                    $this->encryptIV = $xor;
                    if ($start = Tools::strlen($plaintext) & 7) {
            ...
            case CRYPT_DES_MODE_CFB:
                if (!empty($buffer['xor'])) {
                    ...
                    $start = Tools::strlen($ciphertext);
                    $buffer['encrypted'].= $ciphertext;
                    $buffer['xor'] = substr($buffer['xor'], Tools::strlen($ciphertext));
                } else {
                    $ciphertext = '';
                    $iv = $this->encryptIV;
                    $start = 0;
                ...
                for ($i = $start; $i < Tools::strlen($plaintext); $i+=8) {
                    $block = substr($plaintext, $i, 8);
                    $xor = $this->_processBlock($iv, CRYPT_DES_ENCRYPT);
                    $iv = $block ^ $xor;
                    if ($continuousBuffer && Tools::strlen($iv) != 8) {
                        $buffer = array(
                            'encrypted' => $iv,
                            'xor' => substr($xor, Tools::strlen($iv))
                        );
                    }
                    $ciphertext.= $iv;
                }
    ...
    {
        if ($this->paddable) {
            $ciphertext = str_pad($ciphertext, (Tools::strlen($ciphertext) + 7) & 0xFFFFFFF8, chr(0));
        }
        ...
        if ( CRYPT_DES_MODE == CRYPT_DES_MODE_MCRYPT ) {
            ...
            if ($this->mode != 'ncfb') {
                ...
                if (Tools::strlen($this->debuffer)) {
                    $plaintext = $ciphertext ^ substr($this->decryptIV, Tools::strlen($this->debuffer));

                    $this->debuffer.= substr($ciphertext, 0, Tools::strlen($plaintext));
                    if (Tools::strlen($this->debuffer) == 8) {
                        $this->decryptIV = $this->debuffer;
                        $this->debuffer = '';
                        mcrypt_generic_init($this->demcrypt, $this->keys, $this->decryptIV);
                    }
                    $ciphertext = substr($ciphertext, Tools::strlen($plaintext));
                } else {
                    $plaintext = '';
                }

                $last_pos = Tools::strlen($ciphertext) & 0xFFFFFFF8;
                $plaintext.= $last_pos ? mdecrypt_generic($this->demcrypt, substr($ciphertext, 0, $last_pos)) : '';

                if (Tools::strlen($ciphertext) & 0x7) {
                    if (Tools::strlen($plaintext)) {
        ...
        switch ($this->mode) {
            case CRYPT_DES_MODE_ECB:
                for ($i = 0; $i < Tools::strlen($ciphertext); $i+=8) {
            ...
            case CRYPT_DES_MODE_CTR:
                ...
                if ($this->continuousBuffer) {
                    $this->decryptIV = $xor;
                    if ($start = Tools::strlen($ciphertext) % 8) {
                        $buffer = substr($key, $start) . $buffer;
                    }
                }
                break;
            case CRYPT_DES_MODE_CFB:
                if (!empty($buffer['ciphertext'])) {
                    $plaintext = $ciphertext ^ substr($this->decryptIV, Tools::strlen($buffer['ciphertext']));
                    $buffer['ciphertext'].= substr($ciphertext, 0, Tools::strlen($plaintext));
                    if (Tools::strlen($buffer['ciphertext']) == 8) {
                        $xor = $this->_processBlock($buffer['ciphertext'], CRYPT_DES_ENCRYPT);
                        $buffer['ciphertext'] = '';
                    }
                    $start = Tools::strlen($plaintext);
                ...
                for ($i = $start; $i < Tools::strlen($ciphertext); $i+=8) {
                    $block = substr($ciphertext, $i, 8);
                    $plaintext.= $block ^ $xor;
                    if ($continuousBuffer && Tools::strlen($block) != 8) {
                        $buffer['ciphertext'].= $block;
                        $block = $xor;
                    } else if (Tools::strlen($block) == 8) {
    ...
    public function _unpad($text)
    {
        ...
        $length = ord($text[Tools::strlen($text) - 1]);
        ...
        switch ($this->paddingMode) {
            ...
            default : {
                    $padLength = 0;
                    for ($i = Tools::strlen($text) - 1; $i >= 0; $i--) {
                        if ($text[$i] != chr(0)) {
                            break;
                        }
                        $padLength++;

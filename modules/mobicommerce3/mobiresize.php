	...
	public function __construct()
	{
		...
		if($this->isURL){
			...
			if(!$this->localImage)
			{
				...
				{
					$_src_path = '/';
					for($_src_i=1; $_src_i < count($_src); $_src_i++)
					{
						if(strpos($_src[$_src_i], '-') !== FALSE && strpos($_src[$_src_i], '_'))
						{
							$_src = $_src[$_src_i];
							break;
						}
						...
						{
							$_src_path .= $_src[$_src_i].'/';
						}
					}

					//$_src = $_src[1];
					$_src = explode('-', $_src);
					$this->src = $_src_path.'img/p/'.implode('/', str_split($_src[0])).'/'.implode('-', $_src).'.jpg';
					//echo $this->src;exit;
					$this->localImage = $this->getLocalImagePath($this->src);
				}
			}
			// customized by yash - upto here

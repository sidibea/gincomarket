...
abstract class elFinderVolumeDriver {
	...
	public function scandir($hash) {
		...
		if($dir['read']){
			$path = $this->decode($hash);
			$result = $this->getScandir($path);
		}
		else 
			$result = $this->setError(elFinder::ERROR_PERM_DENIED);
	...
	protected function decode($hash) {
		if (strpos($hash, $this->id) === 0) {
			...
			$abspath = $this->_abspath($path);
						
			return $abspath;  //$this->root.($path == DIRECTORY_SEPARATOR ? '' : DIRECTORY_SEPARATOR.$path); 
		}
	...
	protected function getScandir($path) {
		...
		$currentPaths = $this->dirsCache[$path];

...
define('WPS', 1);	// Website payments standard
define('HSS', 2);	// Hosted solution
define('ECS', 4);   //Expres chceckout
...
class PayPal extends PaymentModule
{
	...
	public $module_key = '1e84c5c684aaaaf065051c67ac4c2d00';
	...
	public function getContent()
	{
		...
		$this->context->smarty->assign(array(
			...
			'PayPal_fees_percent' => (int)Configuration::get('PAYPAL_FEESPERCENT'),
			'PayPal_fees_percent_name' => Configuration::getInt('PAYPAL_FEESPERCENT_NAME', $this->context->language->id),
			'PayPal_fees_percent_desc' => Configuration::getInt('PAYPAL_FEESPERCENT_DESC', $this->context->language->id),
	...
	public function hookPayment($params)
	{
		...
		//Showing fee and precentage on payment's list
		...
		$this->decimals = $currency_decimals * _PS_PRICE_DISPLAY_PRECISION_;
				...
				//d($total);
				$value_fees_percent = (int)Configuration::get('PAYPAL_FEESPERCENT');
				$percentd = floatval($value_fees_percent / 100);
				$total_cart_products = Tools::ps_round($total, $this->decimals);		
				
				$total_cart_shipping = Tools::ps_round($shipping_cost_wt, $this->decimals);
				$ammount_extra_tpl = Tools::ps_round(($total_cart_products * $percentd), $this->decimals);
		//END
			...
			$this->context->smarty->assign(array(
				...
				'PayPal_fees_percent' => (int)Configuration::get('PAYPAL_FEESPERCENT'),
				'PayPal_ammount_extra_tpl' => $ammount_extra_tpl,
	...
	private function _postProcess()
	{
		...
		if (Tools::isSubmit('submitPaypal'))
		{
			...
			elseif ($this->_preProcess())
			{
				...
				Configuration::updateValue('PAYPAL_FEESPERCENT', trim(Tools::getValue('fees_percent')));
				//Configuration::updateValue('PAYPAL_FEESPERCENT_DESC', trim(Tools::getValue('fees_percent_desc')));

			//language sorting out in precentage name
			$message_trads = array();
			foreach ($_POST as $key => $value)
				if (preg_match('/fees_percent_name_/i', $key))
				{
					$id_lang = preg_split('/fees_percent_name_/i', $key);
					$message_trads[(int)$id_lang[1]] = $value;
				}
			Configuration::updateValue('PAYPAL_FEESPERCENT_NAME', $message_trads, true);
			//end
			
			//language sorting out in precentage desc
			$message_trads2 = array();
			foreach ($_POST as $key => $value)
				if (preg_match('/fees_percent_desc_/i', $key))
				{
					$id_lang = preg_split('/fees_percent_desc_/i', $key);
					$message_trads2[(int)$id_lang[1]] = $value;
				}
			Configuration::updateValue('PAYPAL_FEESPERCENT_DESC', $message_trads2, true);
			//end

...
class PaypalExpressCheckout extends Paypal
{
	...
	public function setExpressCheckout()
	{
		...
		// Precentage fee by PrestaDev.pl
...
if (isset($parsed_data['path']))
	...
	private function _setPaymentDetails(&$fields)
	{
		...
		// Set fees precentage by PrestaDev.pl
		$this->setFeesPrecentage($fields, $index, $total, $taxes);
		///
	...
	//Adding precentage fee to amount by PrestaDev.pl
	private function setFeesPrecentage(&$fields, &$index, &$total)
	{
			
				
				$fees_percent_id = "1";
				$fees_percent_name = Configuration::get('PAYPAL_FEESPERCENT_NAME', $this->context->language->id);
				$fees_percent_description = Configuration::get('PAYPAL_FEESPERCENT_DESC', $this->context->language->id);
				$value_fees_percent = (int)Configuration::get('PAYPAL_FEESPERCENT');
				$value_fees_percent = Tools::ps_round($value_fees_percent, $this->decimals);
				///
				...
				$fields['L_PAYMENTREQUEST_0_NUMBER'.++$index] = $fees_percent_id;

				$fields['L_PAYMENTREQUEST_0_NAME'.$index] = $fees_percent_name;
				if (isset($fees_percent_description) && !empty($fees_percent_description))
					$fields['L_PAYMENTREQUEST_0_DESC'.$index] = substr(strip_tags($fees_percent_description), 0, 120).'...';


				/* It is a extra precentage to add, so we add product with this amount*/
				$fields['L_PAYMENTREQUEST_0_AMT'.$index] = Tools::ps_round((($total_cart_products + $total_cart_shipping) * $percentd), $this->decimals);
				$fields['L_PAYMENTREQUEST_0_QTY'.$index] = 1;

<?php
class VerticalMegaMenusLibraries{
    ...
    static function getModuleLangById($id, $langId, $shopId){
        $moduleLang =  DB::getInstance()->getRow("Select * From "._DB_PREFIX_."verticalmegamenus_module_lang Where module_id = $id AND `id_shop` = '$shopId' AND `id_lang` = '$langId'");
        if(!$moduleLang)
        {
			$moduleLang = array();
			$moduleLang['name'] = '';
    ...
    static function getMenuLangById($id, $langId, $shopId){
        $itemLang =  DB::getInstance()->getRow("Select * From "._DB_PREFIX_."verticalmegamenus_menu_lang Where menu_id = $id AND `id_shop` = '$shopId' AND `id_lang` = '$langId'");
        if($itemLang) return $itemLang;
        else return null;
    }
	static function getGroupLangById($id, $langId, $shopId){
        $itemLang =  DB::getInstance()->getRow("Select * From "._DB_PREFIX_."verticalmegamenus_group_lang Where group_id = $id AND `id_shop` = '$shopId' AND `id_lang` = '$langId'");
        if($itemLang) return $itemLang;
        else return null;
    }
    static function getMenuItemLangById($id, $langId, $shopId){		
        $itemLang =  DB::getInstance()->getRow("Select * From "._DB_PREFIX_."verticalmegamenus_menu_item_lang Where menuId = $id AND `id_shop` = '$shopId' AND `id_lang` = '$langId'");
        if($itemLang) return $itemLang;
        else return null;
    }
    ...
    public static function getMenuIconSrc($image = '', $check = false){
        if($image && file_exists(_PS_MODULE_DIR_.'verticalmegamenus/images/icons/'.$image))
            return _PS_BASE_URL_.__PS_BASE_URI__.'modules/verticalmegamenus/images/icons/'.$image;
        else
            if($check == true) 
                return '';
            else
                return _PS_BASE_URL_.__PS_BASE_URI__.'modules/verticalmegamenus/images/icons/default.jpg'; 
    }
    public static function getMenuBannerSrc($image = '', $check = false){
        if($image && file_exists(_PS_MODULE_DIR_.'verticalmegamenus/images/banners/'.$image))
            return _PS_BASE_URL_.__PS_BASE_URI__.'modules/verticalmegamenus/images/banners/'.$image;
        else
            if($check == true) 
                return '';
            else
                return _PS_BASE_URL_.__PS_BASE_URI__.'modules/verticalmegamenus/images/banners/default.jpg'; 
    }
	...
	public static function getProducts_Sales($id_lang, $arrCategory = array(), $params = null, $total=false, $limit=0, $offset = 0){
		...
		ORDER BY `sales` DESC Limit '.$offset.', '.$limit;
	...
	static function getProducts_Price($id_lang, $arrCategory = array(), $params = null, $total = false, $limit=0, $offset = 0){
		$context = Context::getContext();
        $where = "";
        if($arrCategory) $catIds = implode(', ', $arrCategory);        
        if($params){
		...
		if($total == true){
			$sql = 'SELECT COUNT(p.id_product)
				...
				LEFT JOIN `'._DB_PREFIX_.'product_lang` pl
					ON p.`id_product` = pl.`id_product`
					AND pl.`id_lang` = '.(int)$id_lang.Shop::addSqlRestrictionOnLang('pl').'				
				WHERE product_shop.`id_shop` = '.(int)$context->shop->id.' AND product_shop.`active` = 1 AND p.`visibility` != \'none\' '.$where;				
				return (int) DB::getInstance(_PS_USE_SQL_SLAVE_)->getValue($sql);				
		}
        ...
        $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, MAX(product_attribute_shop.id_product_attribute) id_product_attribute, product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity, pl.`description`, pl.`description_short`, pl.`available_now`,
				...
				WHERE product_shop.`id_shop` = '.(int)$context->shop->id.' AND product_shop.`active` = 1 AND p.`visibility` != \'none\' '.$where.' GROUP BY product_shop.id_product ORDER BY p.`price` DESC Limit '.$offset.', '.$limit;

           	$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);            
          	if (!$result) return false;
            return $result;
    }
    static function getProducts_View($id_lang, $arrCategory = array(), $params = null, $total = false, $limit=0, $offset = 0){
    	$context = Context::getContext();
        $where = "";
        if($arrCategory) $catIds = implode(', ', $arrCategory);        
        if($params){
        ...
        $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, MAX(product_attribute_shop.id_product_attribute) id_product_attribute, product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity, pl.`description`, pl.`description_short`, pl.`available_now`,
				...
				LEFT JOIN `'._DB_PREFIX_.'manufacturer` m
					ON m.`id_manufacturer` = p.`id_manufacturer`
				WHERE product_shop.`id_shop` = '.(int)$context->shop->id.' AND product_shop.`active` = 1 AND p.`visibility` != \'none\' '.$where.' GROUP BY product_shop.id_product ORDER BY gv.`total` DESC Limit '.$offset.', '.$limit;
				
           	$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS($sql);            
          	if (!$result) return false;
            return $result;
    }
	static function getProducts_List($id_lang, $productIds=array(), $total=false){
		$context = Context::getContext();
        ...
        $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, MAX(product_attribute_shop.id_product_attribute) id_product_attribute, product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity, pl.`description`, pl.`description_short`, pl.`available_now`,
					...
					il.`legend`, m.`name` AS manufacturer_name, cl.`name` AS category_default,
					DATEDIFF(product_shop.`date_add`, DATE_SUB(NOW(),
					INTERVAL '.(Validate::isUnsignedInt(Configuration::get('PS_NB_DAYS_NEW_PRODUCT')) ? Configuration::get('PS_NB_DAYS_NEW_PRODUCT') : 20).' DAY)) > 0 AS new, product_shop.price AS orderprice
				FROM `'._DB_PREFIX_.'category_product` cp
				LEFT JOIN `'._DB_PREFIX_.'product` p
					ON p.`id_product` = cp.`id_product`
				'.Shop::addSqlAssociation('product', 'p').'
				...
				LEFT JOIN `'._DB_PREFIX_.'manufacturer` m
					ON m.`id_manufacturer` = p.`id_manufacturer`
				WHERE product_shop.`id_shop` = '.(int)$context->shop->id.' AND product_shop.`active` = 1 AND p.`visibility` != \'none\' AND p.id_product IN ('.$ids.') GROUP BY product_shop.id_product';
	...
	static function getProducts_AddDate($id_lang, $arrCategory = array(), $params = null, $total=false, $limit=0, $offset = 0){
		$context = Context::getContext();
        $where = "";
        if($arrCategory) $catIds = implode(', ', $arrCategory);        
        if($params){
        ...
        $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, MAX(product_attribute_shop.id_product_attribute) id_product_attribute, product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity, pl.`description`, pl.`description_short`, pl.`available_now`,
				...
				LEFT JOIN `'._DB_PREFIX_.'manufacturer` m
					ON m.`id_manufacturer` = p.`id_manufacturer`
				WHERE product_shop.`id_shop` = '.(int)$context->shop->id.' AND product_shop.`active` = 1 AND p.`visibility` != \'none\' '.$where.' GROUP BY product_shop.id_product ORDER BY p.`date_add` DESC Limit '.$offset.', '.$limit;
